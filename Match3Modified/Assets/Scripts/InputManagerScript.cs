...
{
	public static InputManagerScript instance; //ADDED this
	...
	[SerializeField] protected GameObject selected = null;
		...
		if(Input.GetMouseButtonDown(0))
		{
			...
			if(collider != null)
			{
				...
				else 
				{
					...
					Vector2 pos1 = GameManagerScript.instance.GetPositionOfTokenInGrid(selected);
					Vector2 pos2 = GameManagerScript.instance.GetPositionOfTokenInGrid(collider.gameObject);

					//ADDED this if they are the same icon, do not swap it
					if (selected.GetComponent<SpriteRenderer>() != collider.gameObject.GetComponent<SpriteRenderer>())
					{	 
						//if they're next to each other, we swap them
						if(Mathf.Abs(pos1.x - pos2.x) + Mathf.Abs(pos1.y - pos2.y) == 1) //ADDED THIS from Mathf.Abs((pos1.x - pos2.x) + (pos1.y - pos2.y)) == 1 to Mathf.Abs(pos1.x - pos2.x) + Mathf.Abs(pos1.y - pos2.y) == 1
						{
							MoveTokensScript.instance.SetupTokenExchange(selected, pos1, collider.gameObject, pos2, true);
						}
					}
